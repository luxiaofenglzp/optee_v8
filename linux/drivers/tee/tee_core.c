				  ...
				  struct tee_ioctl_buf_data __user *ubuf)
{
	...
	/*
	long unsigned int i;
	printk("Kernel Printk Begin");
	printk("arg.uuid:");
	for(i=0;i<sizeof(arg.uuid);i++)
		printk("%d",arg.uuid[i]);
	
	printk("arg.clnt_uuid:");
	for(i=0;i<sizeof(arg.clnt_uuid);i++)
		printk("%d",arg.clnt_uuid[i]);
	
	printk("arg.clnt_login:%d",arg.clnt_login);
	
	printk("arg.cancel_id:%d",arg.cancel_id);
	
	printk("arg.num_paramss:%d",arg.num_params);
	printk("Kernel Printk End");
	*/
